...
namespace PTAUpdater.Helpers.Common
{
    public class DapperUtility<T>
    {
        ...
        public static async Task<T> GetSingleObjectAsync(IDbConnection dbConnection, string procName, CommandType commandType)
        {
            T retVal;
            try
            {
                ...
                {                    
                    retVal = await SqlMapper.QueryFirstOrDefaultAsync<T>(con, procName, null, commandType: commandType);
                }
                return retVal;
            }
            catch (Exception ex)
        ...
        public static async Task<DataTable> GetObjectAsync(IDbConnection dbConnection, IDictionary<string, DapperParameterWrapper> paras, string procName, CommandType commandType, bool basisPosting = default)
        {
            try
            {
                _dbConnection = dbConnection;
